...
def Main():
    ...
    fc1_mut = tf.reshape(tf.tile(fc1_mut,[1,BATCH_SIZE]),(18,fc1_unit,BATCH_SIZE))
    fc1_mut = tf.transpose(tf.transpose(fc1_mut,[2,1,0]),[0,2,1]) #(batch_size,18,fc1_unit)
    ...
    lstm_h = tf.reshape(tf.tile(lstm_h,[1,18]),(BATCH_SIZE,HIDDEN_SIZE,18))
    ...
    # two fc layer
    fc1_lstm_l = tf.reshape(states[1][1], (-1,HIDDEN_SIZE))
    ...
    a_sum = tf.reshape(tf.tile(a_atten,[1,fc2_unit]),(BATCH_SIZE,18,fc2_unit))
        ...
        for i in range(10):
            ...
            for j in range(10):
                for k in range(n // batch_size + 1):
                    ...
                    logger.info('iterator :' + str(j * n + k * batch_size) + 'train loss is {:.4f}'.format(np.sqrt(mean_squared_error(y_true, y_))))
                    ...
                    if k % 50 == 0:
                        ...
                        logger.info('iterator :' + str(j) + 'val loss is {:.4f}'.format(np.sqrt(mean_squared_error(y_true, y_))))
                        if val_error > error:
                            val_error = error
                            saver.save(sess, 'model/my-model', global_step = j * n + k * batch_size)
                            # test
                            source_test, target_test_mete, target_test_aqi = dp.test()
                            dist_angle_test = mergeDistAngle(target_station, source_station)
                            dist_angle_test = dist_angle_test.reshape(dist_angle_test.shape[0], dist_angle_test.shape[1])

                            result = sess.run(y_hat,
                                              feed_dict = {source_input: source_test, target_input_mete: target_test_mete,
                                                           source_dist_angle: dist_angle_test,BATCH_SIZE:877})
            ...
            ans += ans_tmp
        logger.info('average rmse of random 10 {:.4f}'.format(ans / 10))
...
if __name__ == '__main__':
    ...
    # repeat = np.repeat(arr, repeats = 4, axis = 1).reshape(2,2,4).transpose([0,2,1])
    # print(repeat)
