# @Time : 2018/10/6 下午7:30 
# @Author : Kaishun Zhang 
# @File : svm_simple_smo.py 
# @Function: 用smo 来求解 svm
...
def get_w(alphas, dataset, labels):
    ...
    return np.dot(alphas * labels, dataset)
...
def simple_smo(dataset, labels, C, max_iter):
    ...
    def f(x):
        #李航 p127
        ret = np.dot(alphas * labels, np.dot(dataset, np.array(x).T)) + b
        return ret
...
if '__main__' == __name__:
    # 加载训练数据
    dataset, labels = load_data('/Users/icdi/Desktop/py_ws/MachineLearning/cs229/SVM/testSet.txt')
    # 使用简化版SMO算法优化SVM
    ...
    x1, _ = max(dataset, key=lambda x: x[0]) # 会取 0 列最大值所在的行作为返回的行
    ...
    # plt.show()
    plt.savefig('svm_simple_smo.jpg')
